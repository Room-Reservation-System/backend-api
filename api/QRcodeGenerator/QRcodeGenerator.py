...
class QRCode:
	...
	def generateQRCode(self,color='#000000'):
		...
		version, level, qr_name = amzqr.run(words=self.linkData,version=3,level='H',picture=self.logoPath,colorized=True,contrast=0.5,brightness=1.3,save_name=pathToQRCode)
	...
	def __drawText(self,image,text:str,color:str='#000000',font=None,textType:str=None,textLocation:Tuple[int,int]=None):
		...
		if font is None or font not in fonts:
			font=ImageFont.truetype(font=fonts['public-sans.light'],**textTypes[textType])
